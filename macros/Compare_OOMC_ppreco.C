...
void Compare_OOMC_ppreco() {
  ...
  std::vector<TString> sampleNames= {"AMPT ppreco","Hijing ppreco", "Hydjet ppreco"};
  ...
  for (const auto& histName : histNames) {
    ...
    canvas->SaveAs(Form("/Users/vipulpant/OOanalysis/Plots/OOMC_ppreco_%s.pdf",histName.Data()));  
  }
}
