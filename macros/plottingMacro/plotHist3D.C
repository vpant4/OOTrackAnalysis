...
void plotHist3D() {
  ...
  TFile *f = new TFile("/Users/vipulpant/cernbox/out_test.root");
  TLatex t;
  char ndir[256] = "HITrackCorrections";
  double ptmax = 100.;

  // sim-to-reco hists
  TH3F *hSim = (TH3F*) f->Get(Form("%s/hsim3D",ndir)); hSim->GetYaxis()->SetRangeUser(0.1,ptmax);
  TH3F *hEff = (TH3F*) f->Get(Form("%s/heff3D",ndir)); hEff->GetYaxis()->SetRangeUser(0.1,ptmax);
  TH3F *hMul = (TH3F*) f->Get(Form("%s/hmul3D",ndir)); hMul->GetYaxis()->SetRangeUser(0.1,ptmax);

  // reco-to-sim hists
  TH3F *hRec = (TH3F*) f->Get(Form("%s/hrec3D",ndir)); hRec->GetYaxis()->SetRangeUser(0.1,ptmax);
  TH3F *hFak = (TH3F*) f->Get(Form("%s/hfak3D",ndir)); hFak->GetYaxis()->SetRangeUser(0.1,ptmax);
  TH3F *hSec = (TH3F*) f->Get(Form("%s/hsec3D",ndir)); hSec->GetYaxis()->SetRangeUser(0.1,ptmax);
  ...
  int ptbin10=hSim->GetYaxis()->FindBin(0.1);
  int ptbins=hSim->GetYaxis()->GetNbins();
  cout<<"      "<<hSim->GetYaxis()->GetBinCenter(hSim->GetYaxis()->GetNbins())<<" ";

  int etabin24m=hSim->GetXaxis()->FindBin(-2.39);
  int etabin24p=hSim->GetXaxis()->FindBin(2.39);
  int etabin10m=hSim->GetXaxis()->FindBin(-2.39);
  int etabin10p=hSim->GetXaxis()->FindBin(2.39);

  int occbin0 = hSim->GetZaxis()->FindBin(90);
  int occbin50 = hSim->GetZaxis()->FindBin(150);
  int occbin51 = hSim->GetZaxis()->FindBin(150);
  int occbin100 = hSim->GetZaxis()->FindBin(200);
  ...
  cout << "occbin51: " << occbin51 << "occbin100: " << occbin100 << endl;
  ...
  TH1D* hSimEta2 = (TH1D*) hSim->ProjectionX("hSimEta2",ptbin10,ptbins,occbin51,occbin100,"e");
  TH1D* hEffEta2 = (TH1D*) hEff->ProjectionX("hEffEta2",ptbin10,ptbins,occbin51,occbin100,"e");
  TH1D* hMulEta2 = (TH1D*) hMul->ProjectionX("hMulEta2",ptbin10,ptbins,occbin51,occbin100,"e");
  TH1D* hRecEta2 = (TH1D*) hRec->ProjectionX("hRecEta2",ptbin10,ptbins,occbin51,occbin100,"e");
  TH1D* hFakEta2 = (TH1D*) hFak->ProjectionX("hFakEta2",ptbin10,ptbins,occbin51,occbin100,"e");
  TH1D* hSecEta2 = (TH1D*) hSec->ProjectionX("hSecEta2",ptbin10,ptbins,occbin51,occbin100,"e");
  ...
  TH1D* hSimPt2  = (TH1D*) hSim->ProjectionY("hSimPt2",etabin10m,etabin10p,occbin51,occbin100,"e");
  TH1D* hEffPt2  = (TH1D*) hEff->ProjectionY("hEffPt2",etabin10m,etabin10p,occbin51,occbin100,"e");
  TH1D* hMulPt2  = (TH1D*) hMul->ProjectionY("hMulPt2",etabin10m,etabin10p,occbin51,occbin100,"e");
  TH1D* hRecPt2  = (TH1D*) hRec->ProjectionY("hRecPt2",etabin10m,etabin10p,occbin51,occbin100,"e");
  TH1D* hFakPt2  = (TH1D*) hFak->ProjectionY("hFakPt2",etabin10m,etabin10p,occbin51,occbin100,"e");
  TH1D* hSecPt2  = (TH1D*) hSec->ProjectionY("hSecPt2",etabin10m,etabin10p,occbin51,occbin100,"e");
  
  TH1D* hDumPt = new TH1D("hDumPt",";p_{T} [GeV/c]",80,0.1,300.0); hDumPt->SetMaximum(1.0);
  hDumPt->GetXaxis()->CenterTitle(); hDumPt->GetYaxis()->SetTitleOffset(1.8); hDumPt->SetTitle("|#eta| < 2.4");
  ...
  gEffEta->SetMarkerStyle(107);
  gEffEta->SetMarkerSize(1.8);
  ...
  gEffPt->SetMarkerSize(1.8);
  gEffPt->SetMarkerStyle(107);
  ...
  gEffEta2->SetMarkerSize(1.8);
  gEffEta2->SetMarkerStyle(108);
  ...
  gEffPt2->SetMarkerStyle(108);
  gEffPt2->SetMarkerSize(1.8);
  ...
  legEta->AddEntry(gEffEta,"0-10%","lp");
  legEta->AddEntry(gEffEta2,"90-100%","lp");

  TLegend *legPt = new TLegend(0.35,0.2,0.65,0.35);
  legPt->SetFillColor(0); legPt->SetBorderSize(0);
  legPt->AddEntry(gEffPt,"0-10%","lp");
  legPt->AddEntry(gEffPt2,"90-100%","lp");
  ...
  hDumEtaEff->GetYaxis()->SetTitle("Absolute efficiency");hDumEtaEff->GetYaxis()->SetRangeUser(0.1,1.0);
  TH1F* hDumPtEff=(TH1F*) hDumPt->Clone("hDumPtEff"); fixedFontHist1D(hDumPtEff,1.05,1.2);
  hDumPtEff->GetYaxis()->SetTitle("Absolute efficiency");hDumPtEff->GetYaxis()->SetRangeUser(0.1,1.0);
  ...
  saveCanvas(c7, "files", "AbsoluteEfficiency3D_fulleta_occ_0_90_PbPb");

  // Multiple Reco
  TGraphAsymmErrors *gMulEta = new TGraphAsymmErrors(); gMulEta->SetName("gMulEta");
  gMulEta->BayesDivide(hMulEta,hSimEta);
  gMulEta->SetMarkerStyle(107);
  gMulEta->SetMarkerSize(1.8);
  ...
  gMulPt->SetMarkerStyle(107);
  gMulPt->SetMarkerSize(1.8);
  ...
  gMulEta2->SetMarkerStyle(108);
  gMulEta2->SetMarkerSize(1.8);
  ...
  gMulPt2->SetMarkerStyle(108);
  gMulPt2->SetLineStyle(4);
  gMulPt2->SetMarkerSize(1.8);
  ...
  //t.DrawLatexNDC(0.15, 0.93, "#bf{CMS} #it{Simulation}");
  //t.DrawLatexNDC(0.6, 0.93, "HYDJET OO 6.8 TeV");
  saveCanvas(c8, "files", "MultipleReconstruction3D_fulleta_occ_0_90_PbPb");
  
  // Fakes
  TGraphAsymmErrors *gFakEta = new TGraphAsymmErrors();  gFakEta->SetName("gFakEta");
  gFakEta->BayesDivide(hFakEta,hRecEta);
  gFakEta->SetMarkerStyle(107);
  gFakEta->SetMarkerSize(1.6);
  ...
  gFakPt->SetMarkerStyle(107);
  gFakPt->SetMarkerSize(1.6);
  ...
  gFakEta2->SetMarkerStyle(108);
  gFakEta2->SetMarkerSize(1.6);
  ...
  gFakPt2->SetMarkerStyle(108);
  gFakPt2->SetMarkerSize(1.6);
  gFakPt2->SetLineStyle(4);
  gFakPt2->SetLineColor(4);
  gFakPt2->SetMarkerColor(4);

  TCanvas* c9 = makeMultiCanvas("c9", "Fake Fraction", 2,1);
  TH1F* hDumEtaFak=(TH1F*) hDumEta2->Clone("hDumEtaMul"); fixedFontHist1D(hDumEtaFak, 1.05,1.2); hDumEtaFak->GetYaxis()->SetRangeUser(0.,0.02);
  hDumEtaFak->GetYaxis()->SetTitle("Fake Reconstruction Fraction");
  TH1F* hDumPtFak=(TH1F*) hDumPt2->Clone("hDumPtMul"); fixedFontHist1D(hDumPtFak, 1.05,1.2); hDumPtFak->GetYaxis()->SetRangeUser(0,0.02);
  ...
  //  t.DrawLatexNDC(0.15, 0.93, "#bf{CMS} #it{Simulation}");
  //t.DrawLatexNDC(0.6, 0.93, "HYDJET OO 6.8 TeV");
  saveCanvas(c10, "files", "SecondaryReconstruction3D_fulleta_occ_0_90_PbPb");
